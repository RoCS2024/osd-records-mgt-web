...
const EmployeeCsSlip = ({ data }) => {
    ...
    const loadTotalHours = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:8080/csSlip/totalCsHours/${data.studentNumber}`, {
                headers: {
    ...
    const handleAddCsReport = useCallback(async (csSlipId, newCsReport) => {
        try {
            ...
            const response = await axios.post(`http://localhost:8080/csreport/addCsReportForSlip/${csSlipId}`, params, {
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
